...
import { calculateRSI, calculateADX } from '../src/indicators.js';

/**
 * אסטרטגיה לפי התמונה:
 * ENTRY (AND):
 * - rsi_above (period=14, threshold=70) timeframe=1m
 * - time_range (startTime=845, endTime=1340) timeframe=1m (בפועל הזמן נבדק על ה-primary)
 * - adx_above (period=14, threshold=25) timeframe=5m
 * EXIT:
 * - rsi_below (period=14, threshold=30) timeframe=1m
 *
 * רץ על 2023, עם פילטר RTH כמו ב-UI כברירת מחדל.
 * חיקוי לוגיקת הבקטסט ב-App.jsx: בדיקה על בר i וביצוע ב-open של הבר הבא.
 */
...
function parseArgs(argv) {
  ...
  if (!args.file) throw new Error('שימוש: node scripts/run_strategy_rsi_time_adx_2023.mjs <csv> [--no-rth] [--limit N]');
  return args;
}
...
// === processData (Close Time label, כמו App.jsx) ===
...
function runBacktest(primaryData, entryOkFn, exitOkFn) {
  ...
  for (let i = 1; i < primaryData.length; i++) {
    ...
    // Exit: נבדק על i, ביצוע ב-open של i+1
    ...
    // Entry
    if (!currentTrade) {
      // block entry if next candle is new session (gap >= 60 minutes)
...
const adx5 = calculateADX(tf5, 14);
const adx5Aligned = alignToPrimary(primary, tf5, adx5);

// תנאים (לפי App.jsx: adx_above משתמש ב-default 25; כאן threshold=25 אז זהה)
const entryOk = (i) => {
  // RSI מעל 70 (1m)
  if (rsi1[i] == null) return false;
  if (!(rsi1[i] > 70)) return false;

  // טווח זמן 08:45-13:40 (על ה-primary)
  const hhmm = hhmmUtcFromTimeSec(primary[i].time);
  if (!(hhmm >= 845 && hhmm <= 1340)) return false;

  // ADX מעל 25 (5m)
  const a = adx5Aligned[i];
...
console.log('=== אסטרטגיה: RSI>70 (1m) + TimeRange 845-1340 + ADX>25 (5m) | Exit RSI<30 (1m) | Year 2023 ===');
