...
> **הערה (עדכון): זה לא באג.** הפילטר RTH שמתחיל ב-`minutes >= 511` (08:31) הוא **תקין ומכוון**.
> NinjaTrader קורא לנרות 1-דקה לפי זמן הסגירה שלהם, כך שהנר שמתחיל ב-08:30 ונגמר ב-08:31
> נקרא "08:31". שתי המערכות מייצרות תוצאות **זהות** כאשר שתיהן רצות על RTH.
...
**עדכון:** ההתנהגות נכונה. NinjaTrader מסמן את הנר הראשון של RTH כ-08:31 (שם הנר = זמן הסגירה). הפילטר `>= 511` תואם בדיוק למנגנון של NinjaTrader.
